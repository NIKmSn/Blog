...
export const allStories = ({ drafts, published, shared }) =>
  [drafts, published, shared ? shared.map(s => s.drafts).flatten_() : undefined]
    .withoutUndef_()
    .flatten_()
...
const updateLastEdit = (oldState, newState) => {
  ...
  return {
    ...newState,
    lastEdit: [sameContent, sameTags, sameTitle].allTrue_()
      ? oldState.lastEdit
      : Date.now()
  }
}
...
const hasLink = value =>
  value.inlines.some(inline => inline.type === BLOCKS.LINK)
  ...
  createReducer(
    {
      ...
      [a.toggleEffect]: (state, type) => {
        try {
          ...
          if (type === BLOCKS.LINK) {
            ...
            return updateLastEdit(state, {
              ...state,
              content: change.value,
              linkPrompt: BLOCKS.LINK
            })
          }
          if (type === BLOCKS.IMAGE) {
            return updateLastEdit(state, {
              ...state,
              content: change.value,
              linkPrompt: BLOCKS.IMAGE
            })
          }
          if (Object.values(MARKS).includes(type)) {
            return updateLastEdit(state, {
              ...state,
              content: change.toggleMark(type).value
            })
          } else {
      ...
      [a.submitLink]: state => {
        ...
        change.call(
          state.linkPrompt === BLOCKS.LINK ? wrapLink : insertImage,
          state.link
        )
        return updateLastEdit(state, {
          ...state,
