...
export function* selectTab({ payload }) {
  ...
  const actionPayload = {
    stories: payload === "shared" ? response.usersDrafts : response.stories,
    tab: payload
  }
...
export function* edit({ payload }) {
  const {
    yourStories: { drafts, published, shared, tab }
  } = yield select()

  const story = allStories({ drafts, published, shared }).find(
